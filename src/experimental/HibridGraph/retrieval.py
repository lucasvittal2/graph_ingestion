from utils import *
from constants import PUBMED_MESH_API_ENDPOINT
from SPARQLWrapper import SPARQLWrapper, JSON
from langchain_core.embeddings import Embeddings
from rdflib import Graph
from typing import List

class PubmedMeshTermAPI:

    def __init__(self, logger: Logger) -> None:
        self._logger = logger
        self._ALTERNATIVE_TERMS_BASE_QUERY_PATH = "src/experimental/HibridGraph/queries/query_alternative_terms.rq"
        self._NARROWER_TERMS_BASE_QUERY_PATH = "src/experimental/HibridGraph/queries/query_narrower_terms.rq"

    def __get_narrower_concepts_for_term(self, term: str) -> List[str]:
        term = sanitize_term(term)  # Sanitize input term
        sparql = SPARQLWrapper(PUBMED_MESH_API_ENDPOINT)
        query = read_text(self._NARROWER_TERMS_BASE_QUERY_PATH, self._logger).format(term=term)
        try:
            self._logger.info("Getting Narrower MeSH terms...")
            ...
            self._logger.info(f"Got  {len(list_concepts)} Narrower MeSH terms.")

            return list_concepts

        except Exception as err:
            self._logger.error(f"Error fetching narrower concepts for term '{term}': {err}")
            raise err

    def get_all_narrower_concepts(self, term, depth=2, current_depth=1):
        term = sanitize_term(term)  # Sanitize input term
        all_concepts = []
        try:
            ...
            if current_depth < depth:
                for concept in narrower_concepts:
                    child_concepts = self.get_all_narrower_concepts(concept, depth, current_depth + 1)
                    all_concepts.extend(child_concepts)

        except Exception as e:
            self._logger.error(f"Error fetching all narrower concepts for term '{term}': {e}")

        return all_concepts

    def get_alternative_terms(self, term: str) -> List[str]:
        term = sanitize_term(term)  # Sanitize input term
        sparql = SPARQLWrapper(PUBMED_MESH_API_ENDPOINT)
        query = read_text(self._ALTERNATIVE_TERMS_BASE_QUERY_PATH, self._logger).format(term=term)
        try:
            self._logger.info("Getting alternative MeSH terms...")
            sparql.setQuery(query)
            sparql.setReturnFormat(JSON)
            results = sparql.query().convert()
            ...
            self._logger.info(f"Got {len(list_triples)} alternative terms.")
            print("\n")

            return list_triples

        except Exception as err:
            self._logger.error(f"Error fetching concept triples for term '{term}': {err}")
            raise err
...
class VectorKGRetriever:
    def __init__(self, embeddings: Embeddings, logger: Logger) -> None:
        ...
        self._logger = logger
        self._pubmed_mesh_api = PubmedMeshTermAPI(logger=self._logger)
        ...
        self._BASE_QUERY_PATH = "src/experimental/HibridGraph/queries/query_graph_knowledge_base.rq"
    ...
    def __query_rdf(self, local_file_path: str, query: str, mesh_terms: List[str], base_namespace:str="http://example.org/mesh/", top_k:int = 10) -> list:
        ...
        self._logger.info("Querying RDF Knowledge graph...\n\n")
        self._logger.info(f"User's query: {query}")
        sparql_query = read_text(self._BASE_QUERY_PATH, self._logger)
        ...
        articles_result = self._article_vector_db.query_contexts(query)
        article_uris = [
        ...
        article_data = {}
        self._logger.info("Converting MeSH terms to a valid URI and query on them...")
        for term in mesh_terms:

            mesh_term_uri = convert_to_uri(term, base_namespace)

            results = g.query(spark_query, initBindings={'meshTerm': mesh_term_uri})
            self._logger.info(f"Queried on Term '{term}' and URI: '{mesh_term_uri}'")
            ...
            self._logger.info(f"Storaged {len(results)} results.")

        self._logger.info("ranking results...")
        ...
        self._logger.info("Ranked result by number of match MeshTerms.")
        self._logger.info(f"Returned {len(top_ranked_articles)} ranked results.")
        return top_ranked_articles

    def get_contexts(self, query: str, graph_path: str, top_k: int = 10) -> list:
        try:
            self._logger.info("Retrieving contexts...")
            mesh_terms_result = self._mesh_terms_vector_db.query_contexts(query)
            retrieved_mesh_terms = [doc.page_content for doc in mesh_terms_result]


            alternative_terms = self._pubmed_mesh_api.get_alternative_terms(query)
            narrower_terms = self._pubmed_mesh_api.get_all_narrower_concepts(query)
            mesh_terms = []
            ...
            self._logger.info(f"Retrieved total top {len(contexts)} contexts successfully !")
            return contexts

        except Exception as err:
            self._logger.error(f"Error during retrieving contexts: \n\n{err}\n\n")
            raise err
